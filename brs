...
/shme/ with a short /e/. snork Brown University, early 1970s. {foo}, {bar},
zot Helsinki University of Technology, Finland. blarg, wibble New Zealand.
toto, titi, tata, tutu France. pippo, pluto, paperino Italy. Pippo /pee'po/
and Paperino /pa-per-ee'-no/ are the Italian names for Goofy and Donald Duck.
aap, noot, mies The Netherlands. These are the first words a child used to
learn to spell on a Dutch spelling board. Of all these, only `foo' and `bar'
are universal (and {baz} nearly so). The compounds {foobar} and `foobaz' also
...
other hand, a language that cannot even be used to write its own compiler is
...
they throw their weight around a lot. :middle-endian: adj. Not {big-endian}
or {little-endian}. Used of perverse byte orders such as 3-4-1-2 or 2-1-4-3,
occasionally found in the packed-decimal formats of minicomputer
manufacturers who shall remain nameless. see {NUXI problem}. Non-US hackers
use this term to describe the American mm/dd/yy style of writing dates
(Europeans write little-endian dd/mm/yy, and Japanese use big-endian yy/mm/dd
for Western dates). :milliLampson: /mil'*-lamp`sn/ n. A unit of talking
speed, abbreviated mL. Most people run about 200 milliLampsons. The eponymous
...
other unflattering ways. This joke expresses a nearly universal attitude
...
in {DEC}'s 3100 workstation series. 4. Acronym for `Meaningless Information
per Second' (a joke, prob. from sense 1). :misbug: /mis-buhg/ n. [MIT] An
...
philosophical change to the structure of the system involved. Many
misfeatures (especially in user-interface design) arise because the
...
directory space and we're stuck with it for now." :Missed'em-five: n.
Pejorative hackerism for AT&T System V Unix, generally used by {BSD}
partisans in a bigoted mood. (The synonym `SysVile' is also encountered.) see
{software bloat}, {Berzerkeley}. :missile address: n. see {ICBM address}.
:miswart: /mis-wort/ n. [from {wart} by analogy with {misbug}] A {feature}
that superficially appears to be a {wart} but has been determined to be the
{Right Thing}. For example, in some versions of the {EMACS} text editor, the
`transpose characters' command exchanges the character under the cursor with
the one before it on the screen, _except_ when the cursor is at the end of a
