...
experiments accessible via the Internet. These are real-time chat forums with
structure; they have multiple `locations' like an adventure game and may
include combat, traps, puzzles, magic, a simple economic system, and the
capability for characters to build more structure onto the database that
represents the existing world. 2. vi. To play a MUD (see {hack-and-slay}).
The acronym MUD is often lower-cased and/or verbed; thus, one may speak of
`going mudding', etc. Historically, MUDs (and their more recent progeny with
names of MU- form) derive from an AI experiment by Richard Bartle and Roy
Trubshaw on the University of Essex's DEC-10 in the early 1980s, and
descendants of that game still exist today (see {BartleMUD}). The title `MUD'
is still copyright to the commercial MUD run by Bartle on British Telecom
(their motto: "You haven't *lived* 'til you've *died* on MUD!"); however,
this did not stop students on the European academic networks from
copying/improving on the MUD concept, from which sprung several new MUDs
...
very successful (among other things, on some versions one was commonly
...
{brain-damaged}. multitask: n. Often used of humans in the same meaning it
...
correct response is either too complicated to enunciate or the speaker has
not thought it out. Often prefaces a longer answer, or indicates a general
reluctance to get into a big long discussion. "Don't you think that we could
improve LISP performance by using a hybrid reference-count transaction
garbage collector, if the cache is big enough and there are some extra cache
bits for the microcode to use?" "Well, mumble... I'll have to think about
...
of some basic form, foo, on a display terminal, and does it using a
relatively simple program; then the program (or the resulting display) is
likely to be referred to as "munching foos" (This is a good example of the
use of the word {foo} as a metasyntactic variable.) munchkin: /muhnch'kin/
[from the squeaky-voiced little people in L. Frank Baum's `The Wizard of Oz']
n. A teenage-or-younger micro enthusiast bashing BASIC or something else
equally constricted. A term of mild derision --- munchkins are annoying but
some grow up to be hackers after passing through a {larval stage}. The term
{urchin} is also used. see also {wannabee}, {bitty box}. mundane: [from SF
fandom] n. 1. A person who is not in science fiction fandom. 2. A person who
is not in the computer industry. In this sense, most often an adjectival
modifier as in "in my mundane life...." mung: /muhng/ alt. `munge' /muhnj/
[in 1960 at MIT, `Mash Until No Good"; sometime after that the derivation
from the {{recursive acronyms}} `Mung Until No Good' became standard] vt. 1.
...
occasionally maliciously. The system only mungs things maliciously; this is a
consequence of {Murphy's Law}. see {scribble}, {mangle}, {trash}, {nuke}.
...
the widespread confusion over the proper spelling of {kluge}). 3. The kind of
beans of which the sprouts are used in Chinese food. (That's their real name!
Mung beans! Really!) Murphy's Law: prov. The correct, *original* Murphy's Law
reads: "If there are two or more ways to do something, and one of those ways
can result in a catastrophe, then someone will do it." This is a principle of
defensive design, cited here because it's usually given in mutant forms which
are less descriptive of the challenges of design for lusers. For example, you
don't make a two-pin plug symmetrical and then label it `THIS WAY UP'; if it
